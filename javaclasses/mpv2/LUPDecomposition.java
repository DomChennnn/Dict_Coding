...
/**
 * LUP Decomposition, P*A = L*U A is N-by-K, P is N-by-N, M is Math.min(N,K), then L is N-by-M and U
 * is M-by-K
 */
 ...
 * ------------------------ */
   ...
   * @serial permutations.
   */
  private final double[] LU;     // verdier i matrisa ordnet kolonnevis
  private int[] perm;
 ...
 * ------------------------ */
  ...
  /**
   * LUP Decomposition, P*A = L*U. Constructor returns a structure to access L, U and P. The
   * dimension of the matrix is assumed to be square and given by the length of the vals array.
   *
   * @param vals The (N*N) matrix elements ordered by columns in an array
   * @throws IllegalArgumentException
   */
  public LUPDecomposition(double[] vals) {
    N = (int)Math.floor(Math.sqrt(vals.length));
    K = vals.length / N;
 ...
 * ------------------------ */
   ...
   * LUP-decomposition is P*A = L*U, where A is N-by-K, P is N-by-N, M is Math.min(N,K), then L is
   * N-by-M and U is M-by-K
   *
  ...
  /**
   * Get number of columns in L and rows in U. LUP-decomposition is P*A = L*U, where A is N-by-K, P
   * is N-by-N, M is Math.min(N,K), then L is N-by-M and U is M-by-K
   *
  ...
  /**
   * Return lower triangular factor as a BandMatrix object. LUP-decomposition is P*A = L*U, where A
   * is N-by-K, P is N-by-N, M is Math.min(N,K), then L is N-by-M and U is M-by-K
   *
   * @return LMatrix
   */
  ...
  /**
   * Return lower triangular factor as a column-ordered array of length N*M. LUP-decomposition is
   * P*A = L*U, where A is N-by-K, P is N-by-N, M is Math.min(N,K), then L is N-by-M and U is
   * M-by-K
   *
   * @return Larray
   */
  ...
  /**
   * Return upper triangular factor as a BandMatrix object. LUP-decomposition is P*A = L*U, where A
   * is N-by-K, P is N-by-N, M is Math.min(N,K), then L is N-by-M and U is M-by-K
   *
   * @return UMatrix
   */
  ...
  /**
   * Return upper triangular factor as a column-ordered array of length M*K. LUP-decomposition is
   * P*A = L*U, where A is N-by-K, P is N-by-N, M is Math.min(N,K), then L is N-by-M and U is
   * M-by-K
   *
   * @return Uarray
  ...
  /**
   * Return pivot permutation vector, P(n,k)=1 when p[n]=k. LUP-decomposition is P*A = L*U, where A
   * is N-by-K, P is N-by-N, M is Math.min(N,K), then L is N-by-M and U is M-by-K
   *
   * @return p
   */
  ...
  /**
   * Return the permutation matrix, P as a PermutationMatrix object. LUP-decomposition is P*A = L*U,
   * where A is N-by-K, P is N-by-N, M is Math.min(N,K), then L is N-by-M and U is M-by-K
   *
  ...
  /**
   * Solve A*X = B, X is returned as an array. An array with the values of X is returned, but B can
   * be any matrix, i.e. a descendant of AllMatrices. This is perhaps a little bit faster than the
   * solveMatrix method which return a SimpleMatrix object.
   *
   * @param B A Matrix with as many rows as A and any number of columns.
