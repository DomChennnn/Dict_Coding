 ...
 * ------------------------ */
  ...
  /**
   * Array for internal storage of elements. Gives position for the ones in each row, A(n,k)==1  <=>
   * pos[n]==k.
   *
 ...
 * ------------------------ */
  ...
  /**
   * Construct an N-by-N permutation matrix from a vector p, A(n,k)==1  <=>  p[n]==k. p must be a
  ...
  /**
   * Construct an n-by-n permutation matrix which will make the given array when the matrix is
   * applied to the sorted sequence, ord = A*ascending(ord). Note that A'*ord will give an ordered
   * sequence, i.e. ascending(ord).
   *
 ...
 * ------------------------ */
  ...
  /**
   * Generate perfect shuffle (stride) permutation matrix. This permutation always start with the
   * first element, i.e. P(0,0)=1. The next element is found by going s positions forward (mod n),
   * and if that element is already used go forward until next unused ek\lement is found.
   *
   * @param n Number of rows and colums.
 ...
 * ------------------------ */
  ...
  /**
   * Multiplies the transposed matrix by an array, y = A'*x. Note that the transposed of a
