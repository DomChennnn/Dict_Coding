...
/**
 * This class is a band diagonal matrix implementation of the AllMatrices superclass. The N-by-K
 * matrix <code>A</code> is represented as an array of column vectors, and for each stored array
 * (column vector) it is also stored how many zeros that preceed this array in the column vector of
 * the matrix. Thus the lengths of the stored column vectors are usualle less than <code>N</code>.
 *
...
public class BandMatrix extends AllMatrices
  ...
  /**
   * Stores the number of zeros in the beginning of each column If this variable is
   * <code>null</code> then a non-sparse storing is used, all column vectors are assumed to start
   * at
   * first position and have length <code>N</code>, i.e. <code>fv[k].length == N</code>.
   */
 ...
 * ------------------------ */
  ...
  /**
   * Construct a <code>N&times;K</code> matrix or dictionary with values from given one-dimensional
   * array (which represents the matrix). Length of the array should be <code>N*K</code>.
   *
   * @param iN   number of rows, i.e. length of column vectors of the matrix.
   * @param iK   number of columns
  ...
  * ------------------------ */
  ...
  /**
   * Replace a column in the dictionary with the given column vector. The column should be of
   * length
