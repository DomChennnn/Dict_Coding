...
// KS: error if m<n, what to do then?
 ...
 * ------------------------ */
  ...
  /**
   * QR Decomposition, computed by Householder reflections. Constructor returns a structure to
 ...
 * ------------------------ */
  ...
  public JamaMatrix getQ() {
    ...
    for (int k = n - 1; k >= 0; k--) {
      ...
      Q[k][k] = 1.0;          // index error if A is 3-by-5
      for (int j = k; j < n; j++) {
        if (QR[k][k] != 0) {
          double s = 0.0;
          for (int i = k; i < m; i++) {
