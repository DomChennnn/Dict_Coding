 ...
 *
 * <p>
 * The class SparseMatrices contains stores the matrix elements in a way well suitable to sparse
 * matrices, i.e. most of the entries are zero. For each non-zero element a structure with 5 fields
 * are stored: the row index, the coluimn index, the value, a pointer to next in same row and a
 * pointer to next in same column. Thus the sparsness factor, i.e. number of non-zero elements
 * divided by total number of elements, should be lower that 0.2 to get any effect, and lower than
 * 0.1 to get significant improvement related to the Matrix-class. Read access, especially to a row
 * or column is effective, while write access, especially to elements which are zero, are slower.
 * <BR> There is a quite large number of methods available for matrices in general and in this
 * class
 * specially, see the AllMatrices class for a general overview. Here is a brief overview for added
 * methods:</P>
 *
 * <P><UL>
 * <LI> Constructors:<BR>
...
public class SparseMatrix extends AllMatrices
  ...
  private final int increment = 100;     // det en ker med nr array utvides
...
/*   public static SparseMatrix identity (int m, int n) {
  ...
  * ------------------------ */
  ...
  /**
   * Set an entry (a value) in the dictionary. Note that the row, i.e. position in the column, is
   * given first, the second argument is the column number i.e. the number of the dictionary atom.
   * We should have: <code>0 <= n < N</code> and <code>0 <= k < K</code>. If any argument is outside
   * legal range nothing is done.
   *
  ...
  public void set(int n, int k, double val) {
    ...
    if (i >= 0) {                    // fanns fra fr
      ...
      if (val == 0.0) {
        ...
        if (b < 0) {firstInRow[n] = a;} else {nextInSameRow[b] = a;}
        // og s kolonna
        ...
        if (b < 0) {firstInColumn[k] = a;} else {nextInSameColumn[b] = a;}
      ...
      if (iNextFree < 0) {
        incrementCapacity(increment);   // ker kapasitet
      }
      i = iNextFree;
      ...
      if (b < 0) {firstInRow[n] = i;} else {nextInSameRow[b] = i;}
      // og s kolonna
      a = firstInColumn[k];       // a : after, elementet etter
      b = -1;                     // b : before, elementet foran
      while (a >= 0) {
      ...
      if (b < 0) {firstInColumn[k] = i;} else {nextInSameColumn[b] = i;}
    }
    return;
  }
  ...
  /**
   * Returns the inner product of two dictionary elements, i.e. matrix column vectors. Legal range
   * is <code>0 <= k1 < K</code> and <code>0 <= k2 < K</code>. If <code>k1</code> or <code>k2</code>
  ...
  public double innerProduct(int k1, int k2) {
    ...
    while ((i1 >= 0) && (i2 >= 0)) {
      ...
      if (nonZeroRow[i1] == nonZeroRow[i2]) {ip += nonZeroValue[i1] * nonZeroValue[i2];}
      if (nonZeroRow[i1] <= nonZeroRow[i2]) {i1 = nextInSameColumn[i1];} else {
        i2 = nextInSameColumn[i2];
      }
    }
    return ip;
  }
  ...
  /**
   * Copy a column of the matrix Legal range of the integer argument is <code>0 <= k < K</code>. If
   * argument is out of range a length <code>N</code> array of zeros should be returned. <br> The
   * corresponding Matlab expression would be: <code>A(:,k+1)</code>.
   *
  ...
  /**
   * Sets argument d to a column of the matrix, i.e. a dictionary element or atom. Legal range of
   * the integer argument is <code>0 <= k < K</code>. If argument is out of range a length
   * <code>N</code> array of zeros should be returned. <br> The corresponding Matlab expression
   * would be: <code>D(:,k+1)</code>.
   *
   ...
   * corresponding Matlab expression would be: <code>A(n+1,:)</code>.
   *
   ...
   * element. An array <code>y</code> is returned. <br> The dimensions are: <code>D</code> is
  ...
  public void transposeTimes(double[] x, double[] y) {
    ...
    for (int i = 0; i < nonZeroCount; i++) {
      if (nonZeroValue[i] != 0.0) {y[nonZeroColumn[i]] += x[nonZeroRow[i]] * nonZeroValue[i];}
    }
    return;
  }
   ...
   * each column is a dictionary element. An array <code>x</code> is returned. <br> The dimensions
   * are: <code>D</code> is
   * <code>N&times;K</code>, <code>x</code> is <code>N&times;1</code>,
   * and <code>y</code> is <code>K&times;1</code>. <br> The corresponding mulitplication in Matlab
   * would be: <code>x = D*y</code>.
   *
  ...
  public void times(double[] y, double[] x) {
    ...
    for (int i = 0; i < nonZeroCount; i++) {
      if (nonZeroValue[i] != 0.0) {x[nonZeroRow[i]] += y[nonZeroColumn[i]] * nonZeroValue[i];}
    }
    return;
  }
