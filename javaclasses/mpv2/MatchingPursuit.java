...
public class MatchingPursuit
{
    ...
    {
        if(i > 100)
            System.out.println("initEkstraVariabler: S-value is too large. Continue but may get short of memory.");
        if(i > maxS1)
        {
            r = new double[i][K];
            T = new int[K];
    ...
    public double[] vsSelectBest(double ad[])
    {
        ...
        if(ad.length != N)
        {
            throw new IllegalArgumentException((new StringBuilder()).append("vsSelectBest: Input argument vector x is not expected length, N=").append(N).toString());
        } else
        {
            vsSelectBest(ad, ad1);
            return ad1;
    ...
    private void vsSelectBest(double ad[], double ad1[])
    {
        ...
        {
            for(int i = 0; i < K; i++)
                ad2[i] = ad2[i] / Math.sqrt(tableInnerProduct(i, i));
    ...
    public double[] vsBMP(double ad[], int i)
    {
        double ad1[] = new double[K];
        if(ad.length != N)
            throw new IllegalArgumentException((new StringBuilder()).append("vsBMP: Input argument vector x is not expected length N=").append(N).toString());
        if(i == 1)
            vsSelectBest(ad, ad1);
        if(i > 1 && i <= 100)
            vsBMP(ad, ad1, i, 2 * i, 1.0000000000000001E-005D, 0);
        return ad1;
    }
    ...
    private void vsBMP(double ad[], double ad1[], int i, int j, double d1, int k)
    {
        ...
        {
            System.out.println((new StringBuilder()).append("vsBMP: NullPointerException s=").append(j1).append(", km=").append(k1).append(", count=").append(l1).toString());
    ...
    public double[] vsOMP(double ad[], int i)
    {
        ...
        if(ad.length != N)
        {
            System.out.println((new StringBuilder()).append("vsOMP: Input argument vector x is not expected length N=").append(N).toString());
            return ad1;
        }
        if(i == 1)
            vsSelectBest(ad, ad1);
        if(i > 100)
            System.out.println((new StringBuilder()).append("vsOMP: Input argument S=").append(i).append(" is to large.").toString());
        if(i > 1 && i <= 100)
            vsOMPorORMP(ad, ad1, i, 1.0000000000000001E-005D, true);
        return ad1;
    }
    ...
    public double[] vsOMP(double ad[], int i, double d1)
    {
        ...
        if(i > 100)
        {
            System.out.println((new StringBuilder()).append("Input argument S=").append(i).append(" is to large, S is set to ").append(Math.min(100, N)).toString());
            i = Math.min(100, N);
        }
        if(i > 1)
            vsOMPorORMP(ad, ad1, i, d1, true);
        return ad1;
    }
    ...
    public double[] vsORMP(double ad[], int i)
    {
        ...
        if(ad.length != N)
        {
            System.out.println((new StringBuilder()).append("vsORMP: Input argument vector x is not expected length N=").append(N).toString());
            return ad1;
        }
        if(i == 1)
            vsSelectBest(ad, ad1);
        ...
        if(i > 1 && i <= 100)
            vsOMPorORMP(ad, ad1, i, 1.0000000000000001E-005D, false);
        return ad1;
    }
    ...
    int vsOMPorORMP2(double ad[], double ad1[], int i, double d1, boolean flag)
    {
        ...
        for(d2 -= d4 * d4; j1 + 1 < i && d2 > d3;)
        {
            ...
            if(d2 + d3 < 0.0D)
            {
                System.out.println((new StringBuilder()).append("vsOMPorORMP: Values not as expected, here n2x=").append(d2).append(", s=").append(j1).append(", normalized = ").append(normalized).append(", do vsBMP instead.").toString());
                normalized = checkNormalized();
                System.out.println((new StringBuilder()).append("and checkNormalized returned ").append(normalized).append(".").toString());
    ...
    public double[] vsPS(double ad[], int i, double d1, int j)
    {
        ...
        if(ad.length != N)
        {
            System.out.println((new StringBuilder()).append("vsPS: Input argument vector x is not expected length N=").append(N).toString());
            return ad1;
        }
        if(d1 <= 0.0D)
            d1 = 9.9999999999999995E-007D;
    ...
    void vsPS(double ad[], double ad1[], int i, double d1, int j)
    {
        ...
        {
            if(++k1 < 0 || k1 >= i)
                System.out.println((new StringBuilder()).append("vsPS: s=").append(k1).append(", beginning MAIN LOOP.").toString());
            if(flag)
            {
                for(int j2 = 0; j2 < K; j2++)
                    d[j2] = T[j2] < 0 ? 0.0D : Math.abs(c[j2]);
    ...
    private int distributeFunction(int i, int j, int k, int ai[])
    {
        ...
        int j2 = findMax(d);
        if(d[j2] <= 0.0D)
            System.out.println((new StringBuilder()).append("Error: distributeFunction  km=").append(j2).append(", d[km]=").append(d[j2]).toString());
        if(l <= 1)
            ai[l1 + j2] = 1;
        else
        if(j + 2 == i)
            ...
            if(d2 > 0.0D)
            {
                ...
                while(l2 > l) 
                {
                    ...
                    for(int j3 = 0; j3 < K; j3++)
                        if(d[j3] > 0.0D && (d[k1] == 0.0D || d[j3] < d[k1]))
                            k1 = j3;
                ...
                if(l2 != l)
                    System.out.println((new StringBuilder()).append("Warning: sumMm = ").append(l2).append(" != ").append(l).append(" = nCombLeft").toString());
            } else
            {
                System.out.println((new StringBuilder()).append("Warning: sum of d array not as expected, sumD = ").append(d2).toString());
                ai[l1 + j2] = 1;
            }
        }
        return j2;
