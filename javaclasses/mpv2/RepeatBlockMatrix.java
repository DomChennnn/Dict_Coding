 ...
 * .
 * </PRE>
 * The matrix M is repeated L times. Let M be of size N-by-K then for each repetition the matrix is
 * translated K positions horisonotally and dN positions vertically or dK positions horisonotally
 * and N positions vertically as illustrated above. That is dN==N or dK==K and 1 <= dN <= N and 1 <=
 * dK <= K. The number of repetitions may be flexible, (L==0). The size (when L>0) of this matrix is
 * (N+(L-1)*dN)-by-(K+(L-1)*dK).
 * <p>
 *
