...
def img_encode(path_in, path_out, Ds, mode=0):
    ...
    if mode == 0:
        ...
        bytes_cnt = write_uints(saved, (info0[0], info0[1]))
        bytes_cnt = write_uints(saved, (info1[0], info1[1], info1[2]))
        bytes_cnt = write_uints(saved, (info2[0], info2[1], info2[2]))
        saved.write(mybits)
        saved.close()
    elif mode == 1:
        # res encode
